...
const drawOneBird = function(id){
	console.log(getBirdById(id).asDOM);
}
			...
			for (const m of bird.presence.split(/[\s\n]?[;,.]\s?/)){
				...
				try {
					  ...
					  try {
						...
						console.log(`Arrival and Departure are (probably) the same:`);
						console.log(`Arrival is ${bird.arrival}, Departure is ${bird.departure}`);
						console.log(`Leaving status as ${status}`);
				 }
